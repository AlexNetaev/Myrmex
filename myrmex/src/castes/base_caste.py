...
class BaseCaste(ABC):
    ...
    def ask_llm(
        ...
        Args:
            ...
            max_retries: Maximale Anzahl der Versuche (überschreibt Default).
            temperature: Die Temperatur (überschreibt Default).
            context_size: Die Context-Size (überschreibt Default).
            model: Das Modell (überschreibt Default).
        ...
        Raises:
            Exception: Wenn das LLM nicht verfügbar ist oder keine gültige
                       Antwort gibt.
        """
        from src.llm.ollama_client import OllamaClient, OllamaConnectionError, OllamaResponseError
        
        # Holen Sie den globalen Ollama-Client (Singleton)
        client = _get_ollama_client()
        
        # Speichern Sie die Originalwerte für die Wiederherstellung
        original_max_retries = client.max_retries
        original_temperature = client.temperature
        original_context_size = client.context_size
        original_model = client.model
        
        # Optionale Parameter überschreiben
        if max_retries is not None:
            client.max_retries = max_retries
        if temperature is not None:
            client.temperature = temperature
        if context_size is not None:
            client.context_size = context_size
        if model is not None:
            client.model = model
        
        try:
            self.logger.debug(
                "[%s] Calling LLM (model=%s, prompt_length=%d)",
                self.caste_name.value if hasattr(self, 'caste_name') else 'unknown',
                client.model,
                len(prompt),
            )
            ...
            # Originale Parameter wiederherstellen
            client.max_retries = original_max_retries
            client.temperature = original_temperature
            client.context_size = original_context_size
            client.model = original_model


# Globale Ollama-Client-Instanz (Singleton)
_ollama_client: "OllamaClient | None" = None


def _get_ollama_client() -> "OllamaClient":
    """
    Gibt den globalen Ollama-Client zurück (Singleton).
    Erstellt den Client beim ersten Aufruf.
    """
...
# Importiere OllamaClient für den Typ-Hint
