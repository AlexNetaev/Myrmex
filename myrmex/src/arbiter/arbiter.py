...
class Arbiter:
    ...
    def run_cycle(self) -> ArbiterPlan:
        """
        Führt einen vollständigen Arbiter-Zyklus aus:
        1. Landschaft analysieren
        2. Directive und Ziel-Kristall laden
        3. Entscheidung treffen
        4. Plan schreiben
        
        Returns:
            Der geschriebene ArbiterPlan.
        """
        logger.info("[Arbiter] Starting arbiter cycle.")
        
        # 1. Landschaft analysieren
        landscape = self.landscape_analyzer.analyze()
        logger.info(
            "[Arbiter] Landscape analyzed: %d trails, %d crystals, %d warnings | "
            "sparse=%s, strong_trail=%s, warning_nearby=%s",
            landscape.trail_count, landscape.crystal_count, landscape.warning_count,
            landscape.is_sparse, landscape.has_strong_trail, landscape.has_warning_nearby,
        )
        ...
        next_action, reasoning, loop_priorities = self.decision_engine.decide(
            landscape=landscape,
            target_crystal=target_crystal,
            current_plan=current_plan,
        )
        logger.info("[Arbiter] Decision: %s | %s", next_action.value, reasoning)
